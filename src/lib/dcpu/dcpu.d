...
struct vm_t
{
	uint16_t[8] registers;    ///< The 8 virtual machine registers: A, B, C, X, Y, Z, I, J.
	...
	uint16_t[RAM_SIZE] ram;   ///< The virtual machine's main RAM segment.
	uint16_t dummy;           ///< A dummy position that is used internally to silently redirect assignments.
	uint16_t[NUM_IRQS] irq;   ///< The interrupt queue.
...
vm_t* vm_create();
void vm_flash(vm_t* vm, uint16_t[0x10000] memory);
